...
if not BOT_TOKEN:
    raise ValueError("Ошибка не устновлено")
...
def cmd_cancel(message):
    if get_role(message.from_user.id) != "client":
        return # Или отправить сообщение о недостатке прав, потом подумаю
...
def cmd_delete(message):
    ...
    try:
        ...
        if row and row["booked_by"]: 
             # Если слот был забронирован
             # Здесь можно добавить логику -- уменьшать лимит у пользователя или нет?
             # вообще просто удаляем слот, не влияя на лимиты.
             # Если нужно уменьшать, нужно вызвать decrement_limit_on_cancellation с данными пользователя row["booked_by"] и датой row["date"].
             # Надо подумать вообще смогу релизовать ил инет
             pass
